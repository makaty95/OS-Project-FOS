...
void env_set_priority(int envID, int priority)
{
	...
	struct Env* curr_process ;
	envid2env(envID, &curr_process, 0);

	if(curr_process->priority == priority) return;

	int lk_holding = holding_kspinlock(&ProcessQueues.qlock);
	if (!lk_holding) acquire_kspinlock(&ProcessQueues.qlock);


	if(curr_process->env_status == ENV_READY) {
		sched_remove_ready(curr_process);
		curr_process->priority = priority;
		sched_insert_ready(curr_process);
	} else curr_process->priority = priority;


	lk_holding = holding_kspinlock(&ProcessQueues.qlock);
	if (lk_holding) release_kspinlock(&ProcessQueues.qlock);

...
void allocate_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//	cprintf("allocate_user_mem called with size = %d\n", size);
	//	cprintf(" allocate_user_mem uheapPageAllocStart = %p, uheapPageAllocBreak = %p\n", uheapPageAllocStart, uheapPageAllocBreak);
	//	cprintf("allocate_user_mem uheapPageAllocStart = %p, uheapPageAllocBreak = %p\n", uheapPageAllocStart, uheapPageAllocBreak);

	int num_of_pages = (size + PAGE_SIZE - 1) / PAGE_SIZE;
	for (uint32 page = 0; page < num_of_pages; ++page) {

		uint32 va = (virtual_address + (page * (uint32)PAGE_SIZE));

		// check page table
		uint32* ptr_page_table;
		int status = get_page_table(e->env_page_directory, va, &ptr_page_table);
		if(status == TABLE_NOT_EXIST) {
			ptr_page_table = (uint32*)create_page_table(e->env_page_directory, va);
		}

		// 	no actual page allocation

		// mark the page
		pt_set_page_permissions(e->env_page_directory, va, PERM_USER | PERM_WRITEABLE | PERM_UHPAGE, 0);
...
//		return;
	...
	for (uint32 page = 0; page < num_of_pages; ++page) {
		...
		//---------------------------update PERM_UHPAGE bit--------------------------------
		// remove the mark bit
		pt_set_page_permissions(e->env_page_directory, va, 0, PERM_USER | PERM_WRITEABLE | PERM_UHPAGE);
		//-----------------------------------------------------------------------------------------------
	...
	LIST_FOREACH_SAFE(ws_element, &(e->page_WS_list), WorkingSetElement) {
		...
		if((ws_element->virtual_address) >= virtual_address &&
				(ws_element->virtual_address) < (virtual_address + size)) {

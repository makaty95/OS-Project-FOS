...
inline int pt_get_page_permissions(uint32* directory, uint32 virtual_address )
{
	...
	int state = get_page_table(directory, virtual_address, &ptr_page_table);

	if(ptr_page_table != NULL) {
		int idx_pt = PTX(virtual_address);

		uint32 buffer = ptr_page_table[idx_pt];

		uint32 msk = (1 << 12) - 1;

		uint32 perms = msk&buffer;
		return perms;
	}


	return -1; // has no page table

...
struct Command commands[] =
{
		...
		{"ver", "displays the kernel version", kernel_version , 0},
		...
		{"add", "adds 2 numbers", add, 2},
		{"wm", "writes a buffer into a given address", write_mem, 2},
		//********************************//
...
int write_mem(int number_of_arguments, char **arguments) {

	uint32 phya = (uint32)strtol(arguments[1], 0, 16);

	// translate from phy -> virtual
	char* ptr = (char*)(phya + KERNEL_BASE);

	char* buffer = arguments[2];
	int bufferSize = sizeof(arguments)/sizeof(arguments[0]);
...
int kernel_version(int number_of_arguments, char **arguments) {

	cprintf("Kernel Version: 0.1\n");
	return 0;
}


int add(int number_of_arguments, char **arguments) {

	int a = strtol(arguments[1], 0, 10);
	int b = strtol(arguments[2], 0, 10);
